...
def update_vocabulary(vocab, file_name, is_dict=False, tokenizer=None):
...
def learn_bpe(infile_names, outfile_name, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, tokenizer=None):
    ...
    vocab = Counter()
    for i, f in enumerate(infile_names):
        sys.stderr.write(f'Collecting vocab from {f}\n')
        if tokenizer is None:
            vocab = update_vocabulary(vocab, f, is_dict)
        else:
            vocab = update_vocabulary(vocab, f, is_dict, tokenizer[i])
